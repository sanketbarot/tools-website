/* ════════════════════════════════════════════════
   AI TOOLCOR — UNIVERSAL SITE TRACKER (FIXED)
   Fixes:
   ✅ setInterval(2s) polling REMOVED — storage event is enough
   ✅ console.log removed from production code
   ✅ scrollY used instead of deprecated pageYOffset
   ✅ trackFileProcessed null-safe
════════════════════════════════════════════════ */
...
(function () {
    ...
    function trackVisit() {
        const analytics = getData(KEYS.ANALYTICS, {
            ...
            toolUsage: {}, dailyVisits: {}, firstVisit: Date.now()
        });
        ...
        if (toolId) {
            ...
            if (tool) {
                logActivity({
                    icon: tool.icon || 'fa-eye', bg: 'bg-primary',
    ...
    window.trackFileProcessed = function (toolId, fileCount = 1) {
        ...
        if (tool) {
            logActivity({
                icon: 'fa-file-check', bg: 'bg-success',
    ...
    function hideDisabledTools() {
        ...
        document.querySelectorAll('.tool-card, [data-tool-id]').forEach(card => {
            ...
            if (disabledIds.has(toolId))      card.style.display = 'none';
            else if (enabledIds.has(toolId))  card.style.display = '';
        });

        // Hide nav dropdown items for disabled tools
        document.querySelectorAll('.ndrop-menu a, .nav-links a').forEach(link => {
            const href = link.getAttribute('href') || '';
            if (!href || href === '#' || href.startsWith('http')) return;
            const filename = href.replace('.html', '').split('/').pop();
            if (disabledIds.has(filename))      link.style.display = 'none';
            else if (enabledIds.has(filename))  link.style.display = '';
        });
    ...
    function checkToolStatus() {
        ...
        const tools = getData(KEYS.TOOLS, []);
        const tool  = tools.find(t => t.id === toolId);
        if (tool && !tool.enabled) showDisabledPage(tool);
    }

    /* ─────── SHOW DISABLED TOOL PAGE ─────── */
    function showDisabledPage(tool) {
        if (document.getElementById('aitoolcor-disabled-overlay')) return;
        ...
        overlay.innerHTML = `
            <div style="
                ...
                z-index:99999;">
                <div style="max-width:520px;animation:aitcFadeUp 0.6s ease;">
                    <div style="font-size:90px;margin-bottom:20px;animation:aitcBounce 2s infinite;">🔧</div>
                ...
                <style>
                    @keyframes aitcFadeUp  { from { opacity:0;transform:translateY(20px); } to { opacity:1;transform:none; } }
                    @keyframes aitcBounce  { 0%,100% { transform:translateY(0); } 50% { transform:translateY(-15px); } }
                </style>
            </div>
        `;
    ...
    /* ─────── REMOVE DISABLED OVERLAY (re-enabled) ─────── */
    function removeDisabledOverlay() {
        document.getElementById('aitoolcor-disabled-overlay')?.remove();
    }

    /* ─────── SYNC ON CONFIG CHANGE ─────── */
    function handleUpdate() {
        const current = localStorage.getItem(KEYS.TOOLS_UPDATED) || '0';
        if (current === lastToolsUpdate) return;
        lastToolsUpdate = current;
        ...
        if (tool && !tool.enabled)     showDisabledPage(tool);
        else if (tool && tool.enabled) removeDisabledOverlay();
    ...
    function init() {
        ...
        // ✅ FIX: NO setInterval polling — storage event is sufficient for cross-tab sync
        ...
        // Same-tab sync via custom event (dispatched by AdminStorage.set)
        window.addEventListener('admin-storage-change', (e) => {
            if (e.detail?.key === KEYS.TOOLS) handleUpdate();
        });
    }
