/* ==========================================
   TOOL PAGE JS v5.1 — FIXED
   Fixes:
   ✅ console.log removed (production)
   ✅ passive:true removed from keydown listener (invalid)
   ✅ navLoginBtn uses addEventListener not .onclick
   ✅ scrollY used instead of deprecated pageYOffset
   ✅ Resize handler debounced
   ✅ Ripple keyframe injected only once (id guard)
   ✅ toggleMobileMenu tracks state via aria-expanded
========================================== */
'use strict';
...
/* ── CLOSE ON ESCAPE ── */
// ✅ FIX: removed { passive: true } — keydown cannot be passive
...
window.addEventListener('scroll', function () {
    ...
    requestAnimationFrame(function () {
        ...
        // ✅ FIX: scrollY instead of deprecated pageYOffset
        const scrolled = window.scrollY > 60;
        if (navbar) {
            navbar.style.background        = scrolled ? 'rgba(255,255,255,0.97)' : 'rgba(255,255,255,0.78)';
            navbar.style.boxShadow         = scrolled ? '0 4px 24px rgba(124,58,237,0.10)' : '0 2px 16px rgba(124,58,237,0.06)';
            navbar.style.borderBottomColor = scrolled ? 'rgba(124,58,237,0.12)' : 'rgba(124,58,237,0.08)';
        }
...
document.addEventListener('DOMContentLoaded', function () {
    // ✅ FIX: keyframe injected only once
...
/* ── AUTH CHECK ── */
// ✅ FIX: addEventListener instead of btn.onclick
(function checkAuth() {
    try {
